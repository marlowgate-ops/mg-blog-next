...
function top(mode: string) {
  const list = [...brokers].filter((b: any)=> b?.state !== 'preparing')
  list.sort((a: any, b: any) => {
    if (mode === 'total') return (b?.score ?? 0) - (a?.score ?? 0)
    const ax = a?.subs?.[mode] ?? 0
    const bx = b?.subs?.[mode] ?? 0
    return bx - ax
  })
  return list[0]
}
  ...
  return new ImageResponse(
    (
      <div style={{
        ...
        height: '100%',
        display: 'flex',
        background: 'linear-gradient(135deg,#0f172a,#0b2b3a)',
        color: '#e6f9ff',
        fontSize: 44,
        padding: '48px 56px',
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
          <div style={{fontSize:24,opacity:.7}}>blog.marlowgate.com/{path}</div>
