...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <article>
      ...
      {post.description ? (
        <p className="text-neutral-600 mb-6">{post.description}</p>
      ) : null}
      <div className="text-xs text-neutral-500 mb-8">
        <span>公開: {new Date(post.date).toLocaleDateString('ja-JP')}</span>
        {post.lastmod ? <span className="ml-3">更新: {new Date(post.lastmod).toLocaleDateString('ja-JP')}</span> : null}
      ...
      {post.tags?.length ? (
        <div className="mt-8 text-sm flex flex-wrap gap-2">
          {post.tags.map(t => (
            <a key={t} className="rounded-full border px-3 py-1 hover:bg-neutral-50 dark:hover:bg-neutral-800" href={`/tags/${encodeURIComponent(t)}`}>{t}</a>
          ))}
        </div>
      ) : null}
      <PostFooterCTA />
