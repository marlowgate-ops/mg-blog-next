...
function Stars({ value }: { value: number }) {
  ...
  return (
    <span className={s.stars} aria-label={`評価 ${n} / 5`}>
...
function Bar({ value }: { value: number }) {
  ...
  return (
    <div className={s.scoreBar}>
      <div className={s.scoreBar_inner} style={{ width: `${w}%` }} />
    </div>
  );
}
  ...
  return (
    <article className={`${s.rankingCard} ${isPreparing ? s.isPreparing : ""}`}>
      <div className={s.rankingCard_left}>
        ...
        <div className={s.rankingCard_logo} aria-hidden>
          {initials(brand)}
        </div>
        <div className={s.rankingCard_brand}>
          <strong>{brand}</strong>
        </div>
      </div>
      <div className={s.rankingCard_main}>
        <div
          className={s.rankingCard_score}
          aria-label={`総合スコア ${score}/5`}
        >
          <Bar value={score} />
          <span className={s.scoreText}>{score.toFixed(1)} / 5</span>
        </div>
        {subs && (
          <div className={s.scoreRows}>
            ...
            <div className={s.scoreRow}>
              ...
              <div className={s.scoreBar}>
                <span
                  style={
                    { "--w": `${(subs.app / 5) * 100}%` } as React.CSSProperties
                  }
                ></span>
              </div>
            </div>
      ...
      <div className={s.rankingCard_cta}>
        <a
          className={`${s.btn} ${isPreparing ? s.btn_disabled : ""}`}
          href={isPreparing ? undefined : ctaHref}
          {...(isPreparing ? { "aria-disabled": true } : {})}
        >
